...
- **親子ヒントリレー**（2人）… 都道府県47／世界の国旗44／ことば64／りか60／れきし145／えいご268（2026-09 に 国旗・ことば・りか・れきしを 約2倍に）。ヒントは スマホを持つ人が 声に出して 読むので ルビつきで 出す
...
- 起動時に匿名ログイン（`signInAnonymously`）し、サーバーの記録を取りこんで 新しいほうを採用する
- **記録はブラウザごと**。ログインが無いので、匿名ユーザーIDもそのブラウザの localStorage にある。
  だからスマホとPC、同じPCのChromeとSafariは別人として扱われ、まちがい帳は共有されない。
  サイトデータを丸ごと消す・シークレットウィンドウで開くと匿名ユーザーIDごと消えるため、
  サーバー上の記録は持ち主不明になって戻らない。サーバーに送っているのは、
  記録のキーだけが消えたとき（容量あふれなど）に戻せるようにするため
...
- **ランキング**：結果画面の「ランキングを 見る」／タイトル右上の「ランキング」。ゲーム×学年ごとに上位10＋自分の順位。名前はひらがな2〜6文字（NGワードあり、「ほんとうの なまえは いれないでね」を常時表示）。**同じイベントですでに使われている名前は入力時にはじく**（名前を決めるのは遊んだあとなので、先に遊んだ人の名前はもうサーバーにある。取れなければ素どおり＝名前が決められず点数を落とすほうが困る）。候補（🎲）は「いろ＋いきもの」1400通りから、かぶっていないものを出す。名前は `profiles` に保存され、次回から入力不要（せっていで変更）。**名前を変えると、その人の過去の点数の名前も全部変わる**（`profiles` の trigger が `scores` を書きかえる）
